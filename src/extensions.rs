...
struct LoadedPlugin {
    ...
    /// gets a fresh store.
    instance: Option<(wasmtime::Store<HostState>, Extension)>,
}
...
impl ExtensionHost {
    ...
    fn with_instance<R>(
        &mut self,
        plugin: &str,
        call: impl FnOnce(&mut wasmtime::Store<HostState>, &Extension) -> wasmtime::Result<R>,
    ) -> Result<R, String> {
        ...
        if slot.instance.is_none() {
            ...
            let instance = Extension::instantiate(&mut store, &slot.component, &linker)
                .map_err(|e| format!("link: {e:#}"))?;
            slot.instance = Some((store, instance));
        }
        let (store, instance) = slot.instance.as_mut().expect("just instantiated");
        store.set_epoch_deadline(CALL_DEADLINE_TICKS);
        match call(store, instance) {
            Ok(v) => Ok(v),
            Err(e) => {
    ...
    pub fn render_block(
        ...
        self.with_instance(plugin, |store, instance| {
            instance.call_render_block(store, lang, source, &wit_theme)
        })?
        .map_err(|e| e)
    }
    ...
    pub fn run_command(
        ...
        self.with_instance(plugin, |store, instance| {
            instance.call_run_command(store, id, &input)
        })?
        .map_err(|e| e)
    }
}
