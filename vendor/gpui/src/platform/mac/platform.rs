    ...
    fn set_cursor_style(&self, style: CursorStyle) {
        unsafe {
            ...
            let new_cursor: id = match style {
                ...
                // Upstream used the undocumented NSCursor class methods
                // _windowResizeNorthWestSouthEastCursor and
                // _windowResizeNorthEastSouthWestCursor here. App Store review
                // rejects binaries that merely reference a private selector, so
                // these fall back to the public diagonal-ish cursors. macOS has
                // no public diagonal resize cursor; on native window decorations
                // the system draws its own at the window edges, so this is not
                // visible in practice.
