...
Open a folder and SuperMD quietly indexes the markdown files in it — links, backlinks, and tags, all computed locally and kept fresh as you type. Hidden files, build directories, and anything your `.gitignore` excludes stay out of the graph, and so does a note reached only through a symlink or a hardlink to something outside it — the index never reads outside the workspace it was opened on. Your files stay plain CommonMark on disk; the index is just a cache that rebuilds itself.
